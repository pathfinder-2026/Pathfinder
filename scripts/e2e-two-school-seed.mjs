...
import {
  BASE, GET, POST, PATCH, login, createReporter, MARKER, today, SCHOOLS, buildStudents, inviteMany,
} from "./pathfinder-seed-lib.mjs";

const { pos, neg, skip, section, summarize } = createReporter();
...
async function seedSchool(def) {
  ...
  const teacherSessions = await inviteMany(S, adminTok, "teacher", def.teachers);
  world.sessions.teachers = def.teachers.map((t) => teacherSessions[t.email]);
  const principalCandidate = await inviteMany(S, adminTok, "teacher", [def.principal]);
  ...
  const studentSessions = await inviteMany(S, adminTok, "student", world.students);
