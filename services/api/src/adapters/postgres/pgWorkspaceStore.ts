...
export class PgWorkspaceStore implements WorkspaceStore {
  ...
  async insertTask(t: StudentTask): Promise<void> {
    await this.sql`insert into student_tasks
      (id,school_id,student_id,class_id,teacher_id,type,title,node_id,assessment_id,content_id,due_date,status,completed_at,overdue_notified,baseline,created_at)
      values (${t.id},${t.schoolId},${t.studentId},${t.classId},${t.teacherId},${t.type},${t.title},${t.nodeId},
        ${t.assessmentId},${t.contentId ?? null},${t.dueDate},${t.status},${t.completedAt},${t.overdueNotified},${t.baseline ?? false},${t.createdAt})`;
  }
