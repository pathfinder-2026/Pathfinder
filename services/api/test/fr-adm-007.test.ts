...
describe("FR-ADM-007 assign Principal to campuses", () => {
  it("happy path: a Principal assigned to two campuses gets an aggregated scope within the one school", () => {
    const { ctx } = makeHarness();
    const { school, campus } = seedSchoolWithAdmin(ctx);
    const second = ctx.schools.addCampus(school.id, {
      name: "West Campus",
      academicYear: { name: "2026 West", terms: VALID_YEAR.terms },
    });
    const user = makeUser(ctx, school.id, "principal@springfield.edu");

    ctx.principals.assignPrincipal(user.id, [campus.id, second.campus.id]);

    const scope = ctx.principals.getPrincipalScope(user.id);
  ...
  it("edge — campus not yet configured: assignment is allowed but flagged 'campus setup incomplete'", () => {
    const { ctx } = makeHarness();
    const { school } = seedSchoolWithAdmin(ctx);
    // A campus added without its own academic year is still being set up.
    const incomplete = ctx.schools.addCampus(school.id, { name: "New Campus" });
    expect(incomplete.campus.setupComplete).toBe(false);

    const user = makeUser(ctx, school.id, "principal2@springfield.edu");
    expect(() => ctx.principals.assignPrincipal(user.id, [incomplete.campus.id])).not.toThrow();
  ...
  it("edge — reassignment mid-term revokes access to the previous campus immediately", () => {
    const { ctx } = makeHarness();
    const { school, campus } = seedSchoolWithAdmin(ctx);
    const second = ctx.schools.addCampus(school.id, {
      name: "East Campus",
    ...
    const user = makeUser(ctx, school.id, "principal3@springfield.edu");

    ctx.principals.assignPrincipal(user.id, [campus.id]);
    ctx.principals.reassignPrincipal(user.id, campus.id, second.campus.id);
