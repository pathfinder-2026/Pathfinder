...
describe("M1 approved-content pool gate", () => {
  it("a fully approved item appears; pending variants never do", async () => {
    ...
    const rawUpload = await ctx.content.uploadOne(school.id, t, {
    ...
    const partial = await ctx.content.uploadOne(school.id, t, {
    ...
    const partialItem = (await ctx.contentStore.getContentItem(partial.contentItemId))!;
    await ctx.ingestion.ingest(partialItem.currentVersionId, t);
    await ctx.classification.classify(partial.contentItemId, t); // suggested only
    await ctx.content.attestRights(partial.contentItemId, t);

    const pool = (await ctx.content.approvedPool(school.id)).map((i) => i.id);
  ...
  it("archiving an approved item removes it from the pool", async () => {
    ...
    expect((await ctx.content.approvedPool(school.id)).map((i) => i.id)).toContain(id);

    await ctx.content.archive(id, teacher.user.id, { confirm: true });
    expect(await ctx.content.approvedPool(school.id)).toHaveLength(0);
  });
  ...
  it("adding a new version returns an approved item to draft (out of the pool until re-approved)", async () => {
    ...
    const teacher = await makeTeacher(ctx, school.id, "teacher@springfield.edu");
    const id = await makeApprovedContent(ctx, school.id, teacher.user.id);
    expect(await ctx.content.approvedPool(school.id)).toHaveLength(1);

    await ctx.content.addVersion(id, teacher.user.id, { title: "rev", fileType: "pdf", sizeBytes: 1000, contentHash: testHash("rev") });
    expect(await ctx.content.approvedPool(school.id)).toHaveLength(0);
  });
});
