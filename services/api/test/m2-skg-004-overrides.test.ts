...
describe("FR-SKG-004 teacher overrides", () => {
  ...
  it("happy path: an override is saved and reflected everywhere the mapping is used", async () => {
    ...
    const [mapping] = await ctx.mapping.mapContent(itemId, ["skill-add-fractions"], { source: "ai" });

    const result = await ctx.mapping.overrideMapping(mapping!.id, "skill-convert-fdp", teacherId);
  ...
  it("edge — overriding with existing mastery data prompts to remap history rather than discard it", async () => {
    ...
    await ctx.skillGraphStore.recordMastery(itemId, "skill-add-fractions");

    const prompt = await ctx.mapping.overrideMapping(mapping!.id, "skill-convert-fdp", teacherId);
    ...
    const done = await ctx.mapping.overrideMapping(mapping!.id, "skill-convert-fdp", teacherId, { remapHistorical: true });
  ...
  it("edge — a bulk override applies with a single confirmation", async () => {
    ...
    const m1 = (await ctx.mapping.mapContent(itemId, ["skill-add-fractions"]))[0]!;
    const m2 = (await ctx.mapping.mapContent(itemId, ["skill-solve-linear"]))[0]!;

    const preview = await ctx.mapping.bulkOverride([m1.id, m2.id], "skill-convert-fdp", teacherId);
    expect(preview).toEqual({ requiresConfirmation: true, count: 2 });

    const applied = await ctx.mapping.bulkOverride([m1.id, m2.id], "skill-convert-fdp", teacherId, { confirm: true });
    expect(applied).toEqual({ requiresConfirmation: false, applied: 2 });
    expect((await ctx.skillGraphStore.getMapping(m1.id))?.nodeId).toBe("skill-convert-fdp");
    expect((await ctx.skillGraphStore.getMapping(m2.id))?.nodeId).toBe("skill-convert-fdp");
  });
});
